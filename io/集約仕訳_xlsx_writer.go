...
func (w *集約仕訳XlsxWriter) Save(集約仕訳一覧 []*domain.Ent集計仕訳) error {
	// シートの有無チェックと既存行取得
	var sheetIdx int
	var existingRowsCount int
	if idx, err := w.ef.GetSheetIndex(sheet集約仕訳一覧); err == nil {
		sheetIdx = idx
		existingRows, _ := w.ef.GetRows(sheet集約仕訳一覧)
		existingRowsCount = len(existingRows)
	} else {
		var err error
		sheetIdx, err = w.ef.NewSheet(sheet集約仕訳一覧)
		if err != nil {
			return err
		}
	}
	...
	// 余分な行を削除（データ数が減った場合）
	if existingRowsCount > len(集約仕訳一覧)+1 {
		// 余分な行を1行ずつ削除
		targetRowNum := len(集約仕訳一覧) + 2
		count := existingRowsCount - (len(集約仕訳一覧) + 1)
		for i := 0; i < count; i++ {
			if err := w.ef.RemoveRow(sheet集約仕訳一覧, targetRowNum); err != nil {
				return err
			}
		}
	}
