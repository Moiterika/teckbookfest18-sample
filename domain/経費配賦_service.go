...
// NewService経費配賦 は経費配賦サービスを生成します
func NewService経費配賦(xlsx I按分ルールXlsxIo) *Service配賦 {
	return &Service配賦{
		xlsx: xlsx,
	}
}
...
func (s *Service配賦) Query按分ルール一覧() ([]*Ent按分ルール, error) {
	...
	xlsxRows, err := s.xlsx.Read按分ルール一覧()
	if err != nil {
...
func (s *Service配賦) Execute配賦(集計仕訳一覧 []*Ent集計仕訳, 按分ルール一覧 []*Ent按分ルール) ([]*Ent按分結果明細, error) {
	...
	for _, e := range 集計仕訳一覧 {
		if 按分基準, ok := 按分ルールgroup[newKey按分ルール(e)]; !ok {
			...
			result, err := ProportionalAllocate(e.Fld合計金額, 按分基準, func(x *Ent按分ルール) decimal.Decimal {
				return x.Fld按分基準値
			})
			if err != nil {
